...
if __name__ == '__main__':
    ...
    @app.route('/api/update-child/<childId>', methods=['PUT'])
def update_child(childId):
    """Update a child's details (name, age, grade, etc.)"""
    try:
        data = request.get_json()
        print(f"Update request for child {childId}: {data}")
        
        # Find the child in database
        child_index = next((i for i, c in enumerate(children_db) if c['childId'] == childId), None)
        ...
        for key, value in data.items():
            if key in children_db[child_index]:
                children_db[child_index][key] = value
        ...
        print(f"Child {childId} updated successfully.")
        return jsonify({
            "success": True,
...
@app.route('/api/delete-child/<childId>', methods=['DELETE'])
def delete_child(childId):
    """Delete a child's record from the database"""
    try:
        global children_db
        initial_count = len(children_db)
        children_db = [child for child in children_db if child['childId'] != childId]

        if len(children_db) == initial_count:
            return jsonify({"success": False, "message": "Child not found"}), 404

        save_children_data(children_db)
        print(f"Child {childId} deleted successfully.")
...
if __name__ == '__main__':
    ...
    print(f"\n🚀 MindTrack Flask backend running on http://localhost:{port}")
